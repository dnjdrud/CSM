...
async function createCellAction(formData: FormData) {
  ...
  if (!user) return;
  const topicTags = rawTags ? rawTags.split(",").map((t) => t.trim()).filter(Boolean) : [];
    ...
    <TimelineContainer>
      <section className="px-4 py-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-theme-text">셀</h1>
            <p className="text-sm text-theme-muted mt-0.5">주제별 기도방 · 누구나 참여 가능</p>
          </div>
          <Link
            href="/network/cells"
            className="text-[13px] text-theme-primary hover:opacity-80"
          >
            셀 찾기
          </Link>
        </div>
              ...
              <li key={c.id}>
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="text-xs text-theme-muted shrink-0">👥 {c.memberCount ?? 0}</span>
                  </div>
