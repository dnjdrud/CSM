/**
 * /cell/join/[token] – Server-side invite acceptance page.
 * Validates the token, joins the cell, then redirects to /cell/[id].
 */
...
export default async function CellJoinPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const user = await getCurrentUser();

  if (!user) {
    redirect(`/login?next=/cell/join/${encodeURIComponent(token)}`);
  }

  const result = await acceptInvite(token, user.id);

  if ("cellId" in result) {
    redirect(`/cell/${result.cellId}`);
  }
  ...
  return (
    <TimelineContainer>
      <div className="px-4 py-16 text-center space-y-3">
        ...
        <a href="/cell" className="text-sm text-theme-primary underline">
          셀 목록으로 돌아가기
        </a>
      </div>
    </TimelineContainer>
