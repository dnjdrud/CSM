      ...
      <div className="w-full max-w-sm rounded-2xl bg-theme-surface shadow-xl border border-theme-border">
        <div className="flex items-center justify-between border-b border-theme-border px-5 py-4">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="닫기"
            className="flex h-8 w-8 items-center justify-center rounded-full text-theme-muted hover:bg-theme-surface-2 hover:text-theme-text focus:outline-none focus-visible:ring-2 focus-visible:ring-theme-accent"
          >
        ...
        <div className="max-h-72 overflow-y-auto px-4 py-3">
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <div className="h-5 w-5 animate-spin rounded-full border-2 border-theme-border border-t-theme-text" aria-hidden />
            </div>
          ) : users.length === 0 ? (
            <p className="py-6 text-center text-[13px] text-theme-muted">아직 아무도 없어요.</p>
          ) : (
            <ul className="divide-y divide-theme-border/50">
              {users.map((user) => (
                <li key={user.id} className="flex items-center gap-3 py-2.5">
                  <Link href={`/profile/${user.id}`} onClick={onClose} className="shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-theme-accent rounded-full">
                  ...
                  <div className="min-w-0">
                    <Link
                      ...
                      onClick={onClose}
                      className="block text-[13px] font-medium text-theme-text hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-theme-accent rounded"
                    >
                      {user.name}
                    </Link>
